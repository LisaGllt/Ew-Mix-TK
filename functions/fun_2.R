...
f_In_experiments <- function(Molecule) {
  ...
  if (Molecule == "IMD") {
    ...
    for (i in unique(df_TK_f$ID)) {
      ...
      char_Ww_ngc <- paste(df_TK_i$Weight_ngc, collapse = ",")
      char_Ww_wgc <- paste(df_TK_i$Weight_wgc, collapse = ",")
      ...
      char_i <- paste(
        ...
        paste("    Winit_wgc=", subset(df_TK_i, t == 0)$Weight_wgc, ";", sep = ""),
        paste("    Ci0IMD=", C_worm_t0_IMD, ";", sep = ""),
        paste("    Ce0IMD=", subset(df_TK_i, t == 0)$C_soil_IMD, ";", sep = ""),
        paste("    CclxIMD=", C_clx_IMD, ";", sep = ""),
        paste("    Print(Weight_ngc,", char_tw, ");", sep = ""),
        paste("    Data(Weight_ngc,", char_Ww_ngc, ");", sep = ""),
        paste("    Print(Weight_wgc,", char_tw, ");", sep = ""),
        paste("    Data(Weight_wgc,", char_Ww_wgc, ");", sep = ""),
...
f_Setpoint <- function(Molecule, PrintStep) {
  ...
  Winit <- mean(subset(df_TK_f, t == 0)$w)
...
'
  ...
  text_print <- paste0("# Data
		...
		PrintStep (Weight_wgc,
					0, 42, ", PrintStep, ");
		PrintStep (Weight_ngc,
					0, 42, ", PrintStep, ");
		PrintStep (CiIMD,
					0, 42, ", PrintStep, ");
	}")
...
f_Setpoint_ind <- function(Molecule) {
  text_start_IMD <- '#### Toxicokinetics of Imidacloprid in A. caliginosa
#===============================================


Integrate(Lsodes, 1E-6, 1E-8, 1);

SetPoints("Setpoints_ind.out", "tab_setpoint_ind.out", 0, kuIMD , keIMD, a_growth);

########## Individuals ################################################'
  
  text_start_EPX <- '#### Toxicokinetics of Epoxiconazole in A. caliginosa
#===============================================


Integrate(Lsodes, 1E-6, 1E-8, 1);

SetPoints("Setpoints_ind.out", "tab_setpoint_ind.out", 0, kuEPX , keEPX, a_growth);
...
########## Individuals ################################################'
  ...
  if (Molecule == "IMD") {
    ...
    for (i in unique(df_TK_f$ID)) {
      ...
      char_i <- paste(
        ...
        paste("    Winit_wgc=", subset(df_TK_i, t == 0)$Weight_nog, ";", sep = ""),
        paste("    Ci0IMD=", C_worm_t0_IMD, ";", sep = ""),
        paste("    Ce0IMD=", subset(df_TK_i, t == 0)$C_soil_IMD, ";", sep = ""),
        paste("    CclxIMD=", C_clx_IMD, ";", sep = ""),
      ...
      text_simulations <- paste(text_simulations, char_i, sep = "\n")
    }
  } else if (Molecule == "EPX") {
    text_start <- text_start_EPX
    for (i in unique(df_TK_f$ID)) {
      compteur_exp <- compteur_exp + 1
      ...
      char_i <- paste(
        ...
        paste("    Winit_wgc=", subset(df_TK_i, t == 0)$w, ";", sep = ""),
        paste("    Ci0EPX=", C_worm_t0_EPX, ";", sep = ""),
        paste("    Ce0EPX=", subset(df_TK_i, t == 0)$C_soil_EPX, ";", sep = ""),
        paste("    CclxEPX=", C_clx_EPX, ";", sep = ""),
        paste("    expo=NDoses(", length(df_TK_i$expo), ",", char_expo, ",", char_tw, ");", sep = ""),
  ...
  writeLines(
    text_full,
    here::here(File_path, paste0("TK_", Molecule, "_Setpoint_ind.in"))
  )
}

f_Setpoint_ind_full <- function(Molecule, print_times, nb_split) {
  
  char_final <- vector("list", nb_split)
  text_start <- vector("list", nb_split)
  
  if (Molecule == "IMD"){
    for (k in seq(1,nb_split)) {
      text_start[[k]] <- paste0('#### Toxicokinetics of Imidacloprid in A. caliginosa
#===============================================


Integrate(Lsodes, 1E-6, 1E-8, 1);

SetPoints("Setpoints_ind_full_', k, '.out", "tab_setpoint_ind.out", 0, kuIMD, keIMD, a_growth);

########## Individuals ################################################')
    }
  }else if(Molecule == "EPX"){
    for (k in seq(1,nb_split)) {
      text_start[[k]] <- paste0('#### Toxicokinetics of Epoxiconazole in A. caliginosa
#===============================================


Integrate(Lsodes, 1E-6, 1E-8, 1);

SetPoints("Setpoints_ind_full_', k, '.out", "tab_setpoint_ind.out", 0, kuEPX, keEPX, a_growth);
...
########## Individuals ################################################')
  ...
  if (Molecule == "IMD") {
    for (i in unique(df_TK_f$ID)) {
      ...
      char_i <- paste(
        ...
        paste("    CclxIMD=", C_clx_IMD, ";", sep = ""),
        paste("    expo=NDoses(", length(df_TK_i$expo), ",", char_expo, ",", char_tw, ");", sep = ""),
        paste("    PrintStep(Weight_wgc,", print_times, ");", sep = ""),
        paste("    PrintStep(Weight_ngc,", print_times, ");", sep = ""),
        paste("    PrintStep(CiIMD,", print_times, ");", sep = ""),
        paste("}"),
        sep = "\n"
      )
      idx_split <- ceiling(compteur_exp / (length(unique(df_TK_f$ID)) / nb_split))
      char_final[[idx_split]] <- paste(char_final[[idx_split]], char_i, sep="\n")
    }
  } else if (Molecule == "EPX") {
    for (i in unique(df_TK_f$ID)) {
      compteur_exp <- compteur_exp + 1
  ...
  for (k in seq(1,nb_split)) {
    ...
    fout <- here::here(File_path, paste0("TK_", Molecule, "_Setpoint_ind_full_", k, ".in"))
    writeLines(text_full, fout)
  }
}
...
f_MCSim_ind <- function(path_mod) {
  ...
  cols_other <- setdiff(colnames(tab_setpoint_ind_tmp), c(cols_growth))

  # Réorganisation
  tab_setpoint_ind <- tab_setpoint_ind_tmp[, c(cols_other, cols_growth)]
